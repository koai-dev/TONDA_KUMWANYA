...
import in.gabsinfo.usertimeslotgeofenceattendance.utils.NumberOfMondaysInMonth;

public class CalculateDayWiseSalaryByAdminFragment extends Fragment implements View.OnClickListener, CompoundButton.OnCheckedChangeListener {
    ...
    public void onPrepareOptionsMenu(Menu menu) {
        ...
        MenuItem itemShowAttendancePositionOnMap=menu.findItem(R.id.item_show_attendance_position_on_map);
        if(itemShowAttendancePositionOnMap!=null)
            itemShowAttendancePositionOnMap.setVisible(false);
    }

    @Nullable
    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view=inflater.inflate(R.layout.fragment_calculate_day_wise_salary_by_admin,container,false);
    ...
    private void setToolBar(View view) {
        ...
        if(getActivity() instanceof AdminDashboardActivity) {
            ...
            toolbar.setNavigationOnClickListener(v -> ((AdminDashboardActivity) requireActivity()).onBackPressed());
        }
    }
    ...
    private void init(View view) {
        ...
        if(mPersonModel!=null){
            ...
            mEtAmountPerDay.setText(String.valueOf(mPersonModel.getAmount()));
        ...
        double totalWorkingDays = Double.parseDouble(mTvTotalAttendedDays.getText().toString().trim())
        ...
        double perDayAmount = mPersonModel.getAmount();
                    ...
                    if(strAmountPerDay.trim().length()>0){
                        mTvTotalAmountToPay.setText(
                                CommonMethods.currencyFormatter(
                                        ConstantData.df.format(Double.parseDouble(strAmountPerDay) * Double.parseDouble(s.toString()))));
                    }
                }
            }
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                if(s.toString().trim().length()==0){
                    ...
                    if(strTotalPaidDays.trim().length()>0){
                        mTvTotalAmountToPay.setText(
                                CommonMethods.currencyFormatter(
                                        ConstantData.df.format(Double.parseDouble(strTotalPaidDays) * Double.parseDouble(s.toString()))));
                    }
                }
            }
    ...
    public void onClick(View v) {
        if(v.getId()==R.id.btn_send_salary_slip){
            CommonMethods.handleExternalStoragePermission(requireActivity(),
                    b -> createPDFFile());
        }else if(v.getId()==R.id.rl_total_attended_days){
        ...
        }else if(v.getId()==R.id.rl_total_actual_Working_days){
            CommonMethods.showDialogForInformation(getActivity(),getString(R.string.label_total_absent_days),
                    getString(R.string.msg_info_salary_calc_total_actual_working_days));
        }else if(v.getId()==R.id.ll_total_absent_days){
            CommonMethods.showDialogForInformation(getActivity(),getString(R.string.label_total_absent_days),
                    getString(R.string.msg_info_salary_calc_total_absent_days));
        }else if(v.getId()==R.id.ll_total_paid_days){
        ...
        }else if(v.getId()==R.id.ll_total_amount_to_pay){
    ...
    public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
        if(buttonView.getId()==R.id.cb_total_public_holidays){
            String strTotalPaidDays= mEtTotalPaidDays.getText().toString().trim();
            String strTotalPublicHolidays= mTvTotalPublicHolidays.getText().toString().trim();
            if(isChecked){
                mEtTotalPaidDays.setText(String.valueOf(Double.parseDouble(strTotalPaidDays)+Double.parseDouble(strTotalPublicHolidays)));
            }else{
                mEtTotalPaidDays.setText(String.valueOf(Double.parseDouble(strTotalPaidDays)-Double.parseDouble(strTotalPublicHolidays)));
            }
        }else if(buttonView.getId()==R.id.cb_total_non_working_days){
            ...
            if(isChecked){
                mEtTotalPaidDays.setText(String.valueOf(Double.parseDouble(strTotalPaidDays_)+Double.parseDouble(strTotalNonWorkingDays)));
            }else{
                mEtTotalPaidDays.setText(String.valueOf(Double.parseDouble(strTotalPaidDays_)-Double.parseDouble(strTotalNonWorkingDays)));
            }
        }
